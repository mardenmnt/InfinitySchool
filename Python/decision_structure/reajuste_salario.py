"""
As Organizações Tabajara resolveram dar um aumento de salário aos seus colaboradores e
lhe contrataram para desenvolver o programa que calculará os reajustes.
Faça um programa que recebe o salário de um colaborador e o reajuste segundo o seguinte critério,
baseado no salário atual:

— salários até R$ 280,00 (incluindo) : aumento de 20%
— salários entre R$ 280,00 e R$ 700,00 : aumento de 15%
— salários entre R$ 700,00 e R$ 1500,00 : aumento de 10%
— salários de R$ 1500,00 em diante : aumento de 5%

Após o aumento ser realizado, informe na tela:
— o salário antes do reajuste;
— o percentual de aumento aplicado;
— o valor do aumento;
— o novo salário, após o aumento.
"""

salario = float(input('\nInforme seu salário: ').replace(',', '.'))

if salario <= 280:
    aumento = salario + (salario * 0.20)
    print(f'\nSeu salário era de {salario} reais')
    print(f'O percentual de aumento foi de 20%')
    print('O valor do aumento em reais foi de', round(salario * 0.20, 3), 'reais')
    print(f'Seu novo salário será de {aumento} reais')
elif 280 < salario <= 700:
    aumento = salario + (salario * 0.15)
    print(f'\nSeu salário era de {salario} reais')
    print(f'O percentual de aumento foi de 15%')
    print('O valor do aumento em reais foi de', round(salario * 0.15, 3), 'reais')
    print(f'Seu novo salário será de {aumento} reais')
elif 700 < salario <= 1500:
    aumento = salario + (salario * 0.10)
    print(f'\nSeu salário era de {salario} reais')
    print(f'O percentual de aumento foi de 10%')
    print('O valor do aumento em reais foi de', round(salario * 0.10, 3), 'reais')
    print(f'Seu novo salário será de {aumento} reais')
elif salario > 1500:
    aumento = salario + (salario * 0.05)
    print(f'\nSeu salário era de {salario} reais')
    print(f'O percentual de aumento foi de 5%')
    print('O valor do aumento em reais foi de', round(salario * 0.05, 3), 'reais')
    print(f'Seu novo salário será de {aumento} reais')
